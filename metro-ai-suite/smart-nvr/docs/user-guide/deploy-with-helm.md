...
```bash
helm pull oci://registry-1.docker.io/intel/nvr-event-router --version <version-no>
```
...
```bash
tar -xvf nvr-event-router-<version-no>.tgz
```

This will create a directory named `nvr-event-router` containing the chart files. Navigate to the extracted directory to access the charts.
